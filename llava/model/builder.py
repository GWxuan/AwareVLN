...
from llava.model import AwareVLNModel
    ...
    **kwargs,
    ...
    if is_mm_model(model_path):
        ...
        if ("lora" in model_name.lower() or "dora" in model_name.lower()) and model_base is not None:
            ...
            model = AwareVLNModel.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        ## TODO @yunhao: mind fixing this
            ...
            if "mpt" in model_name.lower():
                ...
                model = AwareVLNModel(config=config, low_cpu_mem_usage=True, **kwargs)
            tokenizer = model.tokenizer
    else:
        # Load language model
