...
class LLaVATrainer(Trainer):
    ...
    def training_step(self, model, inputs):
        """Training step with NaN/Inf/large-loss guard and optional soft-cap (tanh)."""
        model.train()
        ...
        skip_thr = float(getattr(self.args, "skip_loss_threshold", 50.0))
        use_softcap = bool(getattr(self.args, "softcap_loss", False))
        softcap_val = float(getattr(self.args, "softcap_value", 20.0))

        loss_f = loss.detach().float()
        bad = (not torch.isfinite(loss_f).item()) or (loss_f.item() > skip_thr)

        # Sync bad-batch flag across ranks
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            flag = torch.tensor(1 if bad else 0, device=loss.device, dtype=torch.int)
            torch.distributed.all_reduce(flag, op=torch.distributed.ReduceOp.MAX)
            bad = bool(flag.item())

        if bad:
            if getattr(self.args, "local_rank", -1) in (-1, 0):
                print(f"[safe-train] bad batch @step={self.state.global_step}, loss={loss_f.item():.4f} -> zero it")
            loss = torch.zeros((), device=loss.device, dtype=loss.dtype)

        if (not bad) and use_softcap and softcap_val > 0:
            cap = softcap_val
            loss = cap * torch.tanh(loss / cap)
