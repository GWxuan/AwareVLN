...
from llava.data.dataset import add_onetwovla_tokens
...
def train():
    ...
    if resume_path:
        ...
        if training_args.lora_enable:
            ...
            from llava.model.language_model.llava_llama import resolve_model_class

            model_cls = resolve_model_class(config.architectures[0])
    else:
        ## first time training
    ...
    num_added_tokens = add_onetwovla_tokens(tokenizer)
    print(f"Added {num_added_tokens} special tokens.")

    if num_added_tokens > 0:
        # Resize embeddings when new tokens were added
        if tokenizer.pad_token is None:
            smart_tokenizer_and_embedding_resize(
                special_tokens_dict=dict(pad_token=tokenizer.unk_token),
                tokenizer=tokenizer,
                model=model.llm,
            )
        else:
    ...
    # Store special token IDs in model config for forward()
    try:
        REASON_TOKEN_STR = "<BEGIN_OF_REASONING>"
        ACT_TOKEN_STR = "<BEGIN_OF_ACTION>"
        model.config.reason_token_id = tokenizer.convert_tokens_to_ids(
            [dataset.LazyVLNCEWithReasoningDataset.REASON_TOKEN]
        )[0]
        model.config.act_token_id = tokenizer.convert_tokens_to_ids(
            [dataset.LazyVLNCEWithReasoningDataset.ACT_TOKEN]
        )[0]
        mprint(f"OneTwoVLA Tokens added: [REASON]={model.config.reason_token_id}, [ACT]={model.config.act_token_id}")
    except ValueError:
        mprint("Warning: OneTwoVLA Token IDs could not be assigned. Ensure Tokenizer resize succeeded.")
