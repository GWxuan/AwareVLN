    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if "video_path" in sources[0]:
                ...
                video_file = str(sources[0]["video_id"]) + ".mp4"
...
class LazyVLNCEWithReasoningDataset(LazyVLNCEDataset):
    """VLN-CE dataset with interleaved reasoning/action supervision (OneTwoVLA-style sampling)."""
    
    REASON_TOKEN = "<BEGIN_OF_REASONING>"
    ACT_TOKEN = "<BEGIN_OF_ACTION>"
    REASON_PLACEHOLDER = "[PH_REASONING_TOKEN_]" 
    ACTION_PLACEHOLDER = "[PH_ACTION_TOKEN_]"

    def __init__(self, data_path: str, image_folder: str, tokenizer: PreTrainedTokenizer,
                 data_args, training_args, reasoning_json_path: Optional[str] = None):
        super().__init__(data_path, image_folder, tokenizer, data_args, training_args)
        
        self.reasoning_data = self._load_reasoning_data(reasoning_json_path)
        self.use_reasoning = reasoning_json_path is not None
        self.training_args = training_args
        self.rdm = random.Random(self.training_args.seed)
        self.P_ACTION_TO_REASON = 0.4
    ...
    def get_prob(
        ...
        peak_prob: float = 0.75,
        end_prob: float = 0.3,
    ) -> float:
        """Reasoning probability that rises then falls over the step range (peak near the start)."""
        ...
        peak_step = start_step + step_range / 4

        if now_step <= peak_step:
            ratio = (now_step - start_step) / (peak_step - start_step)
            return start_prob + (peak_prob - start_prob) * ratio
        else:
            ratio = (now_step - peak_step) / (end_step - peak_step)
            return peak_prob - (peak_prob - end_prob) * ratio
    
    
    def _load_reasoning_data(self, path: Optional[str]) -> Dict:
        if path is None:
            return {}
        if not os.path.exists(path):
            print(f"ERROR: Reasoning JSON file not found at path: {path}")
            return {}
        ...
        try:
            with open(path, 'r') as f:
                ...
                if isinstance(data_list, list):
                    data_dict = {str(item['id']): item for item in data_list}
                elif isinstance(data_list, dict):
                    data_dict = {str(k): v for k, v in data_list.items()}
                else:
                    print("ERROR: JSON root is neither list nor dict.")
                    return {}
                    
                return data_dict
        except json.JSONDecodeError as e:
            print(f"ERROR: Failed to decode Reasoning JSON at {path}. Check JSON formatting. Error: {e}")
            return {}
        except Exception as e:
            print(f"ERROR: An unexpected error occurred during file loading: {e}")
            return {}
    
    def _get_reasoning_segment(self, episode_id: str, step_idx: int) -> Dict[str, Any]:
        """Return the reasoning segment for the given episode and step (interval lookup)."""
        episode_data = self.reasoning_data.get(episode_id, {})
        ...
        for segment in segments:
            ...
            if end_step == -1:
                end_step_limit = 10000
            else:
                end_step_limit = end_step

            if start_step <= current_step < end_step_limit:
                return segment

        return {
            'reasoning_text': 'No active planning needed.',
            'content': 'No active planning.',
            'updated_content': None,
            'type': 'only_act',
        }
    
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        original_data_sample = self.list_data_dict[i]
        gpt_answer = None
        tokens_to_replace = {}

        episode_id = original_data_sample.get("video_id", str(i))
        episode_id = str(episode_id).strip()
        episode_id_clean = str(episode_id).strip()
        
        if '-' in episode_id_clean:
            parts = episode_id_clean.rsplit('-', 1)
            root_episode_id = parts[0]
            current_step_in_episode = int(parts[1])
        else:
            root_episode_id = episode_id
            current_step_in_episode = 0

        original_instruction = original_data_sample["q"].strip()
        original_action = original_data_sample["a"].strip()

        image_tensor = None
        if not original_data_sample.get("frames"):
            raise ValueError(f"Sample {i} (ID: {episode_id}) missing 'frames' path.")
        
        last_frame_path = original_data_sample["frames"][-1]

        video_dir, frame_file = os.path.split(last_frame_path)
        match = re.search(r"frame_(\d+)\.jpg", frame_file)
        if not match:
                raise ValueError(f"Frame path format error: {last_frame_path}")

        last_frame_idx = int(match.group(1))

        frames = [
            os.path.join(video_dir, f"frame_{j}.jpg")
            for j in range(last_frame_idx + 1)
        ]

        video_paths = [os.path.join(self.image_folder, frame) for frame in frames]
        num_video_frames = self.data_args.num_video_frames

        images, video_loading_succeed = self._load_video(
            video_paths, num_video_frames, self.data_args
        )
        ...
        segment = self._get_reasoning_segment(root_episode_id, step_idx=last_frame_idx)
        
        content_context = segment.get('content')
        updated_content = segment.get('updated_content')
        seg_type = segment.get('type')
        ref_start_step = segment.get('reference_start_step', 0)
        ref_end_step = segment.get('reference_end_step', last_frame_idx+1)
        if ref_end_step < 0 :
            ref_end_step = 1000
        
        if updated_content is not None and updated_content != "" and (seg_type == "newroom" or seg_type == "correction_part2"):
            
            start_step = segment.get('start_step', last_frame_idx)
            end_step_val = segment.get('end_step', last_frame_idx + 10) 
            
            prev_reasoning_prob = self.get_prob(
                start_step, end_step_val, last_frame_idx, 
                start_prob=0.5, peak_prob=0.75, end_prob=0.3,
            )
            
            random_number = self.rdm.random()
            if random_number < prev_reasoning_prob:
                # Input: [Old Planning] -> Output: [New Planning]
                tokens_to_replace[self.REASON_PLACEHOLDER] = self.REASON_TOKEN
                thought_input = content_context or ""
                reasoning_context = thought_input
                gpt_answer = f"{self.REASON_PLACEHOLDER} New reasoning is: '{updated_content}'"
                ref_index = self.rdm.randrange(ref_start_step, min(ref_end_step, last_frame_idx+1))
            else:
                P_OLD_THOUGHT_TO_ACT = 0.3

                if self.rdm.random() < P_OLD_THOUGHT_TO_ACT:
                    # Input: [Old Planning] -> Output: [Action]
                    tokens_to_replace[self.ACTION_PLACEHOLDER] = self.ACT_TOKEN
                    thought_context = content_context or ""
                    reasoning_context = thought_context
                    gpt_answer = f"{self.ACTION_PLACEHOLDER} {original_action}"
                    ref_index = self.rdm.randrange(ref_start_step, min(ref_end_step, last_frame_idx+1))

                else:
                    # Input: [New Planning] -> Output: [Action]
                    tokens_to_replace[self.ACTION_PLACEHOLDER] = self.ACT_TOKEN
                    thought_context = updated_content
                    reasoning_context = thought_context
                    gpt_answer = f"{self.ACTION_PLACEHOLDER} {original_action}"
                    ref_index = self.rdm.randrange(start_step, last_frame_idx+1)
                    
        elif updated_content is not None and updated_content != "" and (seg_type == "correction_part1" or seg_type == "correction_tail"):
            
            start_step = segment.get('start_step', last_frame_idx)
            end_step_val = segment.get('end_step', last_frame_idx + 10) 
            reason_index = segment.get('index') 
            
            if last_frame_idx < reason_index:
                # Input: [Old Planning] -> Output: [New Planning] (in wrong path)
                tokens_to_replace[self.REASON_PLACEHOLDER] = self.REASON_TOKEN
                thought_input = content_context or ""
                reasoning_context = thought_input
                gpt_answer = f"{self.REASON_PLACEHOLDER} New reasoning is: '{updated_content}'"     
            ...
            elif last_frame_idx == reason_index:
                P_OLD_THOUGHT_TO_REA = 0.3 
                random_number = self.rdm.random()
                if random_number < P_OLD_THOUGHT_TO_REA:
                    # Input: [Old Planning] -> Output: [New Planning]
                    tokens_to_replace[self.REASON_PLACEHOLDER] = self.REASON_TOKEN
                    thought_input = content_context or ""
                    reasoning_context = thought_input
        ...
        num_frames = image_tensor.shape[0] if image_tensor is not None else 8

        historical_image_tokens = (DEFAULT_IMAGE_TOKEN + "\n") * (num_frames - 1)
        current_image_token = DEFAULT_IMAGE_TOKEN 

        if reasoning_context:
            related_step = last_frame_idx - ref_index
            final_question_text = (
                f"Imagine you are a robot programmed for navigation tasks. You have been given a video "
                f'of historical observations {historical_image_tokens}, and current observation {current_image_token}\n. Your assigned task is: "{original_instruction}". '
        ...
        conversation = [
            {"from": "human", "value": final_question_text},
        ...
        for placeholder, target_token in tokens_to_replace.items():
            try:
                placeholder_ids = self.tokenizer.encode(placeholder, add_special_tokens=False)
                if len(placeholder_ids) != 1:
                    print(f"Warning: Placeholder '{placeholder}' tokenized into {len(placeholder_ids)} IDs. Skipping replacement.")
                    continue
                ...
                if len(target_token_ids) != 1:
                    print(f"Error: Target Token '{target_token}' tokenized into {len(target_token_ids)} IDs. Check vocabulary setup.")
                    continue
                ...
                for key in ["input_ids", "labels"]:
                    if key in data_dict:
                        ...
                        if isinstance(ids, torch.Tensor):
                            data_dict[key] = torch.where(
                                ids == placeholder_id, 
                                torch.tensor(target_id, device=ids.device), 
                                ids
                            )
                        elif isinstance(ids, list):
                            if len(ids) > 0 and (isinstance(ids[0], list) or isinstance(ids[0], torch.Tensor)):
                                inner_ids = ids[0]
                                if isinstance(inner_ids, list):
                                    data_dict[key][0] = [target_id if id == placeholder_id else id for id in inner_ids]
                                elif isinstance(inner_ids, torch.Tensor):
                                    data_dict[key][0] = torch.where(
                                        inner_ids == placeholder_id,
                                        torch.tensor(target_id, device=inner_ids.device),
                                        inner_ids
                                    )
                            else:
                                data_dict[key] = [target_id if id == placeholder_id else id for id in ids]
                                
            except Exception as e:
                print(f"Token replacement error for {target_token}: {e}. Falling back to original tokenized sequence.")
...
def add_onetwovla_tokens(tokenizer: PreTrainedTokenizer) -> int:
    special_tokens_dict = dict()
    tokens_to_add = []
    
    reason_placeholder = LazyVLNCEWithReasoningDataset.REASON_PLACEHOLDER
    if reason_placeholder not in tokenizer.vocab:
        tokens_to_add.append(reason_placeholder)
        
    action_placeholder = LazyVLNCEWithReasoningDataset.ACTION_PLACEHOLDER
    if action_placeholder not in tokenizer.vocab:
        tokens_to_add.append(action_placeholder)
        
    reason_token = LazyVLNCEWithReasoningDataset.REASON_TOKEN
    if reason_token not in tokenizer.vocab:
        tokens_to_add.append(reason_token)
    
    act_token = LazyVLNCEWithReasoningDataset.ACT_TOKEN
    if act_token not in tokenizer.vocab:
        tokens_to_add.append(act_token)
