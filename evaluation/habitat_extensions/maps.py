...
def drawline(
    img: np.ndarray,
    pt1: Union[Tuple[float, float], List[float]],
    pt2: Union[Tuple[float, float], List[float]],
    color: List[int],
    thickness: int = 1,
    style: str = "dotted",
    gap: int = 15,
) -> None:
    """Draw a line in dotted/dashed/solid style using OpenCV."""
    assert style in ["dotted", "dashed", "filled"]

    # Ensure integer pixel coords for OpenCV
    p1 = (int(pt1[0] + 0.5), int(pt1[1] + 0.5))
    p2 = (int(pt2[0] + 0.5), int(pt2[1] + 0.5))

    if style == "filled":
        cv2.line(img, p1, p2, color, thickness)
        return

    # Distance and degenerate case: same point
    dist = ((p1[0] - p2[0]) ** 2 + (p1[1] - p2[1]) ** 2) ** 0.5
    if dist == 0:
        # For a single point, draw one dot (works for both dotted and dashed styles)
        cv2.circle(img, p1, max(thickness, 1), color, -1)
        return

    # Build sample points along the segment; ensure at least 2 points for dashed pairing
    # num = how many markers to place; ensure >=2 so dashed has pairs
    step = max(gap, 1)
    num = max(int(dist // step) + 1, 2)

    # r ∈ [0,1] inclusive
    rs = np.linspace(0.0, 1.0, num=num)
    pts = [(int(p1[0]*(1-r) + p2[0]*r + 0.5), int(p1[1]*(1-r) + p2[1]*r + 0.5)) for r in rs]

    if style == "dotted":
        for p in pts:
            cv2.circle(img, p, max(thickness, 1), color, -1)
    else:  # dashed
        # draw segments between (0->1), (2->3), ...
