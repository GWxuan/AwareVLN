...
def save_imgs(
    ...
    """Generate video according to specified information. Using a custom
    version instead of Habitat's that passes FPS to video maker.

    Args:
        video_option: list of "tensorboard" or "disk" or both.
        video_dir: path to target video directory.
        images: list of images to be converted to video.
        episode_id: episode id for video naming.
        checkpoint_idx: checkpoint index for video naming.
    ...
    metric_strs = [f"{k}={v:.2f}" for k, v in metrics.items()]
    # video_name = f"episode={episode_id}-ckpt={checkpoint_idx}-" + "-".join(metric_strs)
    video_name = f"{episode_id}"
    ...
    if "tensorboard" in video_option:
        tb_writer.add_video(video_name, np.array(images).transpose(0, 3, 1, 2)[None], fps=fps)
